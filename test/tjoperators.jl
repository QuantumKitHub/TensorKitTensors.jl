...
@testset "Exact Diagonalisation" begin
    ...
    for (particle_symmetry, spin_symmetry) in Iterators.product(particle_syms, spin_syms)
        for slave_fermion in (false, true)
            if (particle_symmetry, spin_symmetry) in Iterators.product(particle_syms, spin_syms)
                t, J = rand(rng, 2)
                H = tjhamiltonian(particle_symmetry, spin_symmetry; t, J, mu = 0.0, L = 2, slave_fermion)
                true_eigenvals = sort(
