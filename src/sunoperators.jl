...
the polarized quadratic Casimir built from the SU(N) generators ``T^a``. It is the
`Trivial` (dense) operator that becomes block diagonal under `SUNIrrep`, with eigenvalue
``\\tfrac{1}{2}[C_2(R) - C_2(\\lambda_1) - C_2(\\lambda_2)]`` on each coupled sector `R`.

`irrep` (or `irreps`) is forwarded to the `SUNIrrep` constructor. Use `irreps` to couple two
different SU(N) irreps.
