...
# Controlled version of a gate: apply `G` to the remaining qubits if the first qubit is |1⟩.
# `one(G)` inherits the scalar type of `G`, unlike `id(space)`, which defaults to `Float64`.
_controlled(elt::Type{<:Number}, G::AbstractTensorMap) =
    proj_0(elt, Trivial) ⊗ one(G) + proj_1(elt, Trivial) ⊗ G
...
@operator CNOT function cnot(elt::Type{<:Number}, ::Type{Trivial})
    return _controlled(elt, pauli_x(elt, Trivial))
end
const CX = cnot
...
@operator CY function cy(elt::Type{<:Number}, ::Type{Trivial})
    ...
    return _controlled(elt, pauli_y(elt, Trivial))
end
...
@operator CH function ch(elt::Type{<:Number}, ::Type{Trivial})
    return _controlled(elt, hadamard(elt, Trivial))
end
...
@operator CP function cphase(elt::Type{<:Number}, ::Type{Trivial}; theta)
    ...
    return _controlled(elt, phase_shift(elt, Trivial; theta))
end
...
@operator CZ function cz(elt::Type{<:Number}, ::Type{Trivial})
    return _controlled(elt, pauli_z(elt, Trivial))
end
...
@operator CS function cs(elt::Type{<:Number}, ::Type{Trivial})
    ...
    return _controlled(elt, s_gate(elt, Trivial))
end
...
@operator TOFFOLI function toffoli(elt::Type{<:Number}, ::Type{Trivial})
    return _controlled(elt, cnot(elt, Trivial))
end
const CCX = toffoli
...
@operator FREDKIN function fredkin(elt::Type{<:Number}, ::Type{Trivial})
    return _controlled(elt, swap(elt, Trivial))
end
const CSWAP = fredkin
