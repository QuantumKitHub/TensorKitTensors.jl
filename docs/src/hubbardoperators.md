...
```math
|0\rangle, \quad |{\uparrow}\rangle = e^\dagger_\uparrow |0\rangle, \quad
|{\downarrow}\rangle = e^\dagger_\downarrow |0\rangle, \quad
|{\uparrow\downarrow}\rangle = e^\dagger_\uparrow e^\dagger_\downarrow |0\rangle,
```
...
```math
\underbrace{|0\rangle,\; |{\uparrow\downarrow}\rangle}_{\text{parity even}}, \quad
\underbrace{|{\uparrow}\rangle,\; |{\downarrow}\rangle}_{\text{parity odd}}
```

This is the order in which `block(op, FermionParity(0))` and `block(op, FermionParity(1))` list their rows and columns — with `Trivial` particle and spin symmetry both blocks are ``2 \times 2``, so there is no single ``4 \times 4`` matrix — and it is the reference order of [`basis_transform`](@ref), which is consequently the identity for `Trivial` symmetry.
...
Rather than relying on it, read it off from the basis transformation: numbering the reference states ``(1, 2, 3, 4) = (|0\rangle, |{\uparrow\downarrow}\rangle, |{\uparrow}\rangle, |{\downarrow}\rangle)``, column ``j`` of `basis_transform(P, S)` has its single nonzero entry in the row that is the dense index of reference state ``j``:
...
| `U1Irrep` | particle-number conservation | ``n \in \{0, 1, 2\}`` | empty ``\mapsto 0``, singly occupied ``\mapsto 1``, doubly occupied ``\mapsto 2`` |
| `SU2Irrep` | ``\eta``-pairing SU(2) | ``\eta \in \{0, 1/2\}`` | (doubly occupied, empty) is the ``\eta = 1/2`` doublet, ordered by descending ``\eta^z = (n-1)/2``; singly occupied has ``\eta = 0`` |
...
| `U1Irrep` | ``S^z`` conservation | ``m \in \{-1/2, 0, +1/2\}`` | ``\uparrow \mapsto +1/2``, ``\downarrow \mapsto -1/2``, empty and doubly occupied ``\mapsto 0`` |
| `SU2Irrep` | full spin SU(2) | ``s \in \{0, 1/2\}`` | the singly occupied states form the ``s = 1/2`` doublet (descending ``m``); empty and doubly occupied are ``s = 0`` |

!!! note "Charge conventions"
    The `U1Irrep` particle charge is the particle number ``n``, not the particle-hole symmetric ``n - 1``, and the `U1Irrep` spin charge is ``S^z = \pm 1/2``, not ``2S^z = \pm 1``.
    The ``\eta``-pairing `SU2Irrep` is built on ``\eta^z = (n-1)/2`` instead, so the particle number itself is *not* an ``\eta``-SU(2) scalar: this is why [`e_num`](@ref) and [`ud_num`](@ref) are unavailable with `SU2Irrep` particle symmetry, while the particle-hole symmetric [`half_ud_num`](@ref) ``= (n_\uparrow - 1/2)(n_\downarrow - 1/2)`` — which equals ``+1/4`` on the ``\eta`` doublet and ``-1/4`` on the singly occupied states — is available for every symmetry combination.

!!! note "Staggered gauge for SU(2) particle symmetry"
    The ``\eta``-pairing SU(2) only commutes with the Hubbard Hamiltonian after the staggered gauge transformation ``c_{j,\sigma} \to i^j c_{j,\sigma}`` on a bipartite lattice.
...
Note that the singlet-pair terms [`singlet_plus_singlet_min_3site`](@ref) and [`singlet_plus_singlet_min_4site`](@ref) are available with `U1Irrep` particle symmetry even though [`singlet_plus`](@ref) and [`singlet_min`](@ref) individually are not: only the product ``\Delta^\dagger \Delta`` conserves the particle number.
